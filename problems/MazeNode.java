...
public class MazeNode extends Node {
	...
	public MazeNode(int row , int col , int size , int Gcost , Node parent , Problem my_parent, int depth){
	...
	public Vector<Node> getChild() {
		...
		if(getCol()>0){
			...
			if(!my_parent.getWalls().contains(first) && ! my_parent.getWalls().contains(second)){
				MazeNode mn = new MazeNode(getRow(), getCol()-1 , size , getGcost()+1 ,this, my_parent,this.depth+1);
				child.add(mn);
			}
		}
		...
		if(getRow()>0){
			...
			if(!my_parent.getWalls().contains(first) && ! my_parent.getWalls().contains(second)){
				MazeNode mn = new MazeNode(getRow()-1, getCol() , size , getGcost()+1 ,this, my_parent,this.depth+1);
				child.add(mn);
			}
		}
		if(getRow()<size-1){
